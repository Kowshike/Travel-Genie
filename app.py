...
def index():
    if request.method == "POST":
        global source, destination, start_date, end_date, budget
        ...
        try:
            ...
            flash("API usage limit exceeded. Try again later.", "danger")
            return redirect(url_for("index"))
        except Exception as e:
            print("Error in fetching weather data:", e)
            flash(f"Error in fetching weather data: {e}", "danger")
            return redirect(url_for("index"))

        # Generate itinerary
        try:
            plan = bard.generate_itinerary(source, destination, start_date, end_date, no_of_day, budget)
            print("Itinerary Generated:", plan)
        except Exception as e:
